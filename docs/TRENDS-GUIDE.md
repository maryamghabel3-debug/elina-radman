...
## ۴) اجرای خودکار روزانه 🌙
تحلیل عمیق حالا به **خط تولید روزانه** (`scripts/generate.py` + workflow `daily-content.yml`) وصله. هر شب به‌ترتیب:
1. عکس‌های ترند رو تحلیل عمیق می‌کنه → `content/trend_visuals.json`
2. ویدیوهای وایرال رو مهندسی معکوس می‌کنه → `content/trend_videos.json`
3. از تحلیل‌ها **ایده‌های ویدیو** می‌سازه → `content/queue/video-*.json`
4. کپشن‌ها رو بر اساس ترند + سیگنال‌های بصری تولید می‌کنه
5. همه رو در تلگرام برای تأیید اعلام می‌کنه

برای صرفه‌جویی در سهمیه API می‌تونی با متغیر `SKIP_DEEP_ANALYSIS=1` این بخش رو موقتاً خاموش کنی.
