...
# Static per-segment visual adjustments (color grade) ranges.
# Applied via a single FFmpeg 'eq' filter, composed after transform and
# before brightness keyframes, to align exposure/contrast/saturation across
# clips generated by different AI models.
VISUAL_ADJUSTMENT_RANGES = {
    "brightness": {"default": 0.0, "min": -1.0, "max": 1.0},
    "contrast": {"default": 1.0, "min": -2.0, "max": 2.0},
    "saturation": {"default": 1.0, "min": 0.0, "max": 3.0},
    "gamma": {"default": 1.0, "min": 0.1, "max": 10.0},
}


def _visual_adjustments_eq_filter(adjustments: Any, segment_index: int = 0) -> Optional[str]:
    """
    Validate visual_adjustments and build the static 'eq' filter string.

    Returns the FFmpeg 'eq' filter for non-identity adjustments, or None when
    the field is absent, empty, or identity (all defaults) so that existing
    filter chains remain byte-for-byte unchanged.

    Raises ValueError with VISUAL_ADJUSTMENTS_INVALID before FFmpeg sees bad data.
    """
    if adjustments is None:
        return None
    if not isinstance(adjustments, dict):
        raise ValueError(
            f"VISUAL_ADJUSTMENTS_INVALID: segment {segment_index}: visual_adjustments must be a dictionary"
        )

    for key in adjustments:
        if key not in VISUAL_ADJUSTMENT_RANGES:
            raise ValueError(
                f"VISUAL_ADJUSTMENTS_INVALID: segment {segment_index}: unknown key '{key}' in visual_adjustments"
            )

    values = {}
    for key, spec in VISUAL_ADJUSTMENT_RANGES.items():
        raw = adjustments.get(key, spec["default"])
        if isinstance(raw, bool) or not isinstance(raw, (int, float)):
            raise ValueError(
                f"VISUAL_ADJUSTMENTS_INVALID: segment {segment_index}: '{key}' must be a number"
            )
        value = float(raw)
        if value < spec["min"] or value > spec["max"]:
            raise ValueError(
                f"VISUAL_ADJUSTMENTS_INVALID: segment {segment_index}: '{key}' must be between "
                f"{spec['min']} and {spec['max']} (got {value})"
            )
        values[key] = value

    # Identity adjustments are a no-op: skip the filter entirely so legacy
    # command output is preserved exactly.
    if (
        values["brightness"] == 0.0
        and values["contrast"] == 1.0
        and values["saturation"] == 1.0
        and values["gamma"] == 1.0
    ):
        return None

    return (
        f"eq=brightness={values['brightness']}:"
        f"contrast={values['contrast']}:"
        f"saturation={values['saturation']}:"
        f"gamma={values['gamma']}"
    )
...
class VideoConcatenator:
    ...
    def build_trim_concat_command(self, segments: List[Dict[str, Any]], output_path: str, keep_audio: bool = False) -> List[str]:
        ...
        # Validate segments, transitions, freezes, transforms, and visual
        # adjustments first. Visual adjustments are validated and resolved to
        # their 'eq' filter string up front (None = absent/identity).
        va_filters: List[Optional[str]] = []
        for i, seg in enumerate(segments):
            va_filters.append(_visual_adjustments_eq_filter(seg.get("visual_adjustments"), i))
        ...
        # Single segment with no trim, no transition, no freeze, no transform,
        # no keyframes, and no visual adjustments: return empty to trigger copy
        if len(segments) == 1:
            seg = segments[0]
            ...
            has_visual_adj = va_filters[0] is not None
            if start == 0.0 and end is None and not has_freeze and not has_transform and not has_keyframes and not has_visual_adj:
                return []

        # Check if there are any non-trivial transitions, freezes, transforms,
        # brightness keyframes, or visual adjustments
        ...
        if not has_complex_operations:
            ...
            logger.debug(f"Generated filter complex: {filter_complex}")
        ...
        # Apply static visual adjustments if requested (composes after transform,
        # before brightness keyframes). Absent/identity adjustments emit no
        # filter, keeping the previous label chain byte-for-byte unchanged.
        # Order: trim -> transform -> visual_adjustments -> brightness_keyframes -> freeze
        for i, seg in enumerate(segments):
            va_eq = va_filters[i]
            if va_eq is not None:
                filter_parts.append(f"[v{i}_trans]{va_eq}[v{i}_adj]")

        # Apply brightness keyframes if requested (composes after visual
        # adjustments, before freeze). With both present, the static and
        # dynamic 'eq' filters are applied as two consecutive filters.
        for i, seg in enumerate(segments):
            keyframe_input = f"v{i}_adj" if va_filters[i] is not None else f"v{i}_trans"
            keyframes = seg.get("brightness_keyframes")
            if keyframes and isinstance(keyframes, list) and len(keyframes) > 0:
                expr = "0.0"
                ...
                filter_parts.append(f"[{keyframe_input}]{brightness_vf}[v{i}_bright]")
            else:
                filter_parts.append(f"[{keyframe_input}]null[v{i}_bright]")
    ...
    def concat_segments(self, segments: List[Dict[str, Any]], output_path: str, keep_audio: bool = False) -> str:
        ...
        # Check if there are any non-trivial transitions, freezes, transforms,
        # keyframes, or visual adjustments
        has_complex_operations = False
        for i in range(len(segments)):
            if i < len(segments) - 1:
                trans = segments[i].get("transition_out")
            ...
            # Validate up front (raises VISUAL_ADJUSTMENTS_INVALID before FFmpeg)
            # and treat non-identity adjustments as a complex operation.
            if _visual_adjustments_eq_filter(segments[i].get("visual_adjustments"), i) is not None:
                has_complex_operations = True
