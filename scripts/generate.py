...
def ensure_weekly_moodboard():
    """Make sure FashionStylist has produced a moodboard for the current ISO
    week before ProductHunter/captions need it. Without this, ProductHunter
    always fell back to a hardcoded default theme because the moodboard file
    was never generated by any pipeline. Wrapped so failures never break the
    daily run.
    """
    try:
        from agents.fashion_stylist import FashionStylist
        ...
        if os.path.exists(moodboard_path):
            try:
                with open(moodboard_path) as f:
                    existing = json.load(f)
                needs_refresh = existing.get("week") != current_week
            except (OSError, json.JSONDecodeError):
                needs_refresh = True

        if needs_refresh:
            vision = FashionStylist().run()
            print(f"   🎨 New weekly moodboard: {vision.get('theme')}")
        else:
            print("   🎨 Weekly moodboard already fresh for this week")
    except Exception as e:
        print(f"   ⚠️  FashionStylist skipped: {e}")
...
def main():
    ...
    # 0) Refresh the weekly fashion moodboard (FashionStylist) if this ISO
    #    week hasn't been styled yet, so ProductHunter always has a real,
    #    current theme instead of silently falling back to a hardcoded one.
    print("🎨 Checking weekly fashion moodboard...")
    ensure_weekly_moodboard()
